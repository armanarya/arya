...
// ============================================================
// ARYA — Personal Optimization Agent · v6
// + Skincare tab with morning/evening rituals
// ============================================================

const STORAGE_KEY = "arya:v6:state";
...
const DEFAULT_STATE: State = {
  ...
  dailyHabits: [
    ...
    { id: "skin-am", label: "AM skincare · full ritual", detail: "Gentle cleanse → Vit C → Alpha Arbutin → Moisturizer → SPF. See Skincare tab.", icon: "sun", xp: 15, section: "skin" },
    { id: "spf-reapply", label: "SPF reapply (noon)", detail: "Critical: you drive 30+ min daily. UVA passes through side windows.", icon: "sun", xp: 10, section: "skin" },
    { id: "skin-pm", label: "PM skincare · full ritual", detail: "Cleanse → Tranexamic + Niacinamide → Moisturizer. See Skincare tab.", icon: "moon", xp: 15, section: "skin" },
    { id: "adapalene", label: "Adapalene (M/W/F nights)", detail: "Differin 0.1%, pea-sized, after moisturizer settles. PM only.", icon: "moon", xp: 10, section: "skin" },
  ...
  oneTimeQuests: [
    ...
    { id: "skincare-haul", label: "Buy tan-reversal stack (~$80)", xp: 60, done: false, priority: 1, notes: "Beauty of Joseon SPF + Alpha Arbutin + Tranexamic Acid + Niacinamide + Differin + CeraVe cleanser. See Skincare tab." },
        ...
        <header className="mb-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <div className="text-[10px] tracking-[0.3em] text-white/30 uppercase mt-1">v6 · Optimization Agent</div>
            </div>
...
const AM_RITUAL: SkincareStep[] = [
  {
    ...
    brand: "CeraVe Hydrating · or LRP Toleriane Hydrating",
    why: "Morning skin doesn't need acid. Keeping barrier intact = SPF works better.",
    how: "Lather 15 sec with lukewarm water. Pat dry — don't rub.",
    price: "$15",
  },
  {
    step: 2,
    product: "Vitamin C serum",
    brand: "The Ordinary Ascorbic Acid 8% + Alpha Arbutin 2%",
    why: "Antioxidant. Blocks new pigment formation. Force-multiplies SPF.",
    how: "4-5 drops, pat into face + neck.",
  ...
  },
  {
    ...
    why: "Gold-standard melanin pathway inhibitor for South Asian skin. Fades existing tan.",
    how: "3-4 drops, pat in.",
    wait: "30 seconds",
    price: "$10",
  },
  {
    step: 4,
    product: "Moisturizer",
    brand: "CeraVe AM Lotion · or LRP Toleriane Double Repair",
    why: "Hydrated skin pigments less than dehydrated skin. Ceramides repair barrier.",
    how: "Quarter-sized amount, face + neck.",
    price: "$17",
  },
  {
    step: 5,
    ...
    why: "PA++++ = max UVA blocking. UVA causes tan (more than UVB). No white cast.",
    how: "TWO full finger-lengths for face + neck + ears + hairline. If it doesn't feel like a lot, it isn't enough.",
    price: "$18",
    flag: "critical",
  },
  {
    ...
    brand: "Same SPF · or Beauty of Joseon Sun Stick for touch-up",
    why: "You drive 30+ min daily. UVA passes through side windows. Once is not enough.",
    how: "Stick over makeup-free spots. Especially LEFT cheek (driver's side).",
    flag: "critical",
  },
];
...
const PM_RITUAL: SkincareStep[] = [
  {
    step: 1,
    product: "Cleanse",
    brand: "Gentle cleanser nightly · LRP Effaclar Medicated 2% SA only 2x/week",
    why: "Daily acid wash damages barrier = more tanning. Demote SA cleanser to weekly maintenance.",
    how: "Lather, rinse. On SA nights, leave on 60 sec before rinsing.",
  },
  {
    step: 2,
    product: "Tranexamic + Niacinamide",
    brand: "The Ordinary Tranexamic 5% (apply first) → Niacinamide 10% (apply second)",
    why: "Two different melanin pathways stacked. Faster fade than any single product. Both gentle enough to layer.",
    how: "3-4 drops tranexamic, pat in. Wait 30 sec. Then 3-4 drops niacinamide.",
    wait: "60 seconds",
    price: "$13 + $8",
  },
  {
    step: 3,
    product: "Moisturizer + Adapalene (M/W/F)",
    brand: "CeraVe PM Lotion + Differin Gel 0.1% on adapalene nights",
    why: "Adapalene accelerates cell turnover — pushes pigmented cells to surface where they slough off. Compresses 6-month timeline to 12 weeks.",
    how: "Moisturizer first. Wait 5 min. On M/W/F: pea-sized Differin to entire face.",
    price: "$15 (Differin) + $17 (CeraVe)",
    flag: "rotating",
  },
];

const SHOPPING_LIST = [
  { name: "Beauty of Joseon Relief Sun SPF 50+ PA++++", price: 18, where: "Amazon / Yesstyle" },
  { name: "CeraVe Hydrating Cleanser", price: 15, where: "Any drugstore" },
  { name: "The Ordinary Ascorbic Acid 8% + Alpha Arbutin", price: 10, where: "Ordinary website / Sephora" },
  { name: "The Ordinary Alpha Arbutin 2% + HA", price: 10, where: "Ordinary website / Sephora" },
  { name: "The Ordinary Tranexamic Acid 5% + HA", price: 13, where: "Ordinary website / Sephora" },
  { name: "The Ordinary Niacinamide 10% + Zinc 1%", price: 8, where: "Ordinary website / Sephora" },
  { name: "Differin Gel 0.1% (Adapalene)", price: 15, where: "Any drugstore — OTC" },
  { name: "CeraVe AM/PM Moisturizer", price: 17, where: "Any drugstore" },
];
...
function WeeklySchedule() {
  ...
  const schedule: Record<string, { am: string; pm: string; adapalene: boolean; saCleanse: boolean }> = {
    Mon: { am: "Full AM", pm: "Full PM + Adapalene", adapalene: true, saCleanse: false },
    Tue: { am: "Full AM", pm: "Full PM", adapalene: false, saCleanse: false },
    Wed: { am: "Full AM", pm: "Full PM + Adapalene", adapalene: true, saCleanse: false },
    Thu: { am: "Full AM", pm: "Full PM + SA cleanse", adapalene: false, saCleanse: true },
    Fri: { am: "Full AM", pm: "Full PM + Adapalene", adapalene: true, saCleanse: false },
    Sat: { am: "Full AM", pm: "Full PM", adapalene: false, saCleanse: false },
    Sun: { am: "Full AM", pm: "Full PM + SA cleanse", adapalene: false, saCleanse: true },
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-white/50 leading-relaxed mb-4">
        Adapalene 3x/week (M/W/F). SA cleanser 2x/week (Thu/Sun) as treatment. AM ritual is daily and identical. PM moisturizer is daily and identical.
      </p>
      <div className="space-y-2">
        {days.map((d, i) => {
          const s = schedule[d];
          return (
            <div key={d} className="grid grid-cols-[60px_1fr] gap-3 items-center border border-white/10 p-3 slide-up" style={{ animationDelay: `${i * 0.04}s` }}>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {s.saCleanse && <span className="text-[10px] px-2 py-1 border border-purple-500/30 bg-purple-950/30 text-purple-200">+ SA cleanse</span>}
              </div>
            </div>
          );
        })}
